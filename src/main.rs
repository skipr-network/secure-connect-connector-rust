...
mod tests {
    ...
    async fn run_heartbeat_succeeds_against_a_correctly_signed_response() {
        ...
        let body = r#"{"connector_id":"c-1","generated_at":"2026-08-27T10:00:00Z","expires_at":"2026-08-27T10:05:00Z","nonce":"n1","policy_bundles":[],"node_list":[]}"#;
        ...
        let result = run_heartbeat(&config, &registry_client, &heartbeat_client).await;
    ...
    async fn run_heartbeat_succeeds_and_warns_when_some_nodes_have_no_wireguard_key_yet() {
        let agent_identity = crypto::generate_keypair();
        let body = r#"{"connector_id":"c-1","generated_at":"2026-08-27T10:00:00Z","expires_at":"2026-08-27T10:05:00Z","nonce":"n1","policy_bundles":[],"node_list":[{"node_id":"n-1","ip_address":"10.0.0.10","wireguard_public_key":null}]}"#;
        let signature = crypto::sign_to_base64(&agent_identity.signing_key, body.as_bytes());
